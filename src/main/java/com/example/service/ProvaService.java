...
public class ProvaService {
	...
	/**
	 * Gera o PDF com cabeçalho + questões + alternativas.
	 */
	public byte[] gerarProvaPDF(Long provaId) throws Exception {
		...
		if (is != null) {
			...
			Image cabecalhoImg = new Image(imageData).setWidth(240).setHeight(100)
		...
		// =================== TÍTULO DA PROVA ===================
		Paragraph titulo = new Paragraph(prova.getTitulo()).setBold().setFontSize(18)
				.setTextAlignment(TextAlignment.CENTER);
		document.add(titulo);
		document.add(new Paragraph("\n"));

		// =================== CAMPOS DE PREENCHIMENTO ===================
		Paragraph camposPreenchimento = new Paragraph().add("Data: ___/___/____     ")
				.add("Nome: __________________________________     ").add("Nota: _________").setFontSize(10)
				.setTextAlignment(TextAlignment.CENTER);
		...
		for (Questao questao : prova.getQuestoes()) {
			...
			Paragraph cabecalho = new Paragraph().add("Questão " + numero).setBold().setFontSize(12);

			if (questao.getDisciplina() != null) {
				cabecalho.add(new Paragraph(" - " + questao.getDisciplina().getNome()).setItalic().setFontSize(9)
			...
			// Enunciado
			document.add(new Paragraph(questao.getEnunciado()).setFontSize(11).setMarginLeft(10).setMarginTop(5));
			...
			if (questao.getOpcoes() != null && !questao.getOpcoes().isEmpty()) {
				for (OpcaoQuestao opcao : questao.getOpcoes()) {
					...
					Paragraph paragrafoOpcao = new Paragraph(textoOpcao).setFontSize(10).setMarginLeft(20)
							.setMarginTop(3);
				...
				document.add(new Paragraph("___________________________________________________").setFontSize(10)
						.setMarginLeft(10));
				document.add(new Paragraph("___________________________________________________").setFontSize(10)
						.setMarginLeft(10));
			}
		...
		// Rodapé
		document.add(
				new Paragraph("\n___________________________________________").setTextAlignment(TextAlignment.CENTER));
		document.add(new Paragraph("Total de questões: " + prova.getQuestoes().size())
				.setTextAlignment(TextAlignment.CENTER).setFontSize(9).setItalic());
